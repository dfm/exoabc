#ifndef _EXOABC_TRANSIT_QUAD_H_
#define _EXOABC_TRANSIT_QUAD_H_

#include <cmath>
#include <cfloat>

#include "exoabc/transit/ellint.h"
...
public:
    ...
    T operator () (const T* const params, const T& p, const T& z0) const
    {
        ...
        if (z <= 0.0 && p <= 1.0) {
            ...
            PAL_AB PAL_CI
            f1 = T(2.0/3.0);
        } else if (z < p && std::abs(z-1.0+p) <= DBL_EPSILON) {
            // M&A -, Pal B_T
            f0 = p2;
            ...
            PAL_AB PAL_CG
            f1 = T(2.0/3.0);
        } else if (std::abs(z-p) <= DBL_EPSILON && z < 1.0-p-DBL_EPSILON) {
            // M&A 5, Pal C
            T t = T(2.0/(9.0*M_PI));
            f0 = p2;
            f1 = T(1.0/3.0);
            ...
            f2 = 1.5*f0*f0;
            k = 2.0*p;
        } else if (std::abs(z-p) <= DBL_EPSILON && std::abs(z-1.0+p) <= DBL_EPSILON) {
            // M&A 6, Pal C_T
            f0 = T(0.25);
            f1 = T(1.0/3.0-4.0/(9.0*M_PI));
            f2 = T(3.0/32.0);
        } else if (std::abs(z-p) <= DBL_EPSILON) {
            // M&A 7, Pal C_G
            PAL_AB PAL_K
            f0 = (p2*k0+k1-sqrt(z2-0.25*(1.0+z2-p2)*(1.0+z2-p2)))/M_PI;
            f1 = T(1.0/3.0);
            ...
            // M&A 3, Pal D
            PAL_AB PAL_CI
        } else if (std::abs(z-1.0+p) <= DBL_EPSILON) {
            // M&A 4, Pal E
            f0 = p2;
            f1 = (2.0/(3.0*M_PI)*acos(1.0-2.0*p)-4.0/(9.0*M_PI)
                  *(3.0+2.0*p-8.0*f0)*sqrt(p*(1.0-p)));
